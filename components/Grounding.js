...
const GroudingExercise = ({ isDarkMode }) => {
    const router = useRouter();
    const { grouding, isLoading, error } = useFetch("search");
    const [Grouding, setGrouding] = useState();
    const themeStyles = getThemeStyles(isDarkMode);

    const renderMeditationCard = ({ item }) => (
        <TouchableOpacity
            style={[styles.container(Grouding, item), { backgroundColor: themeStyles.BackgroundStyle.lightBackground }]}
            onPress={() => handleCardPress(item)}
        >
            <TouchableOpacity style={styles.logoContainer(Grouding, item)}>
                <Image
            ...
            <View style={[styles.infoContainer, { color: themeStyles.TextStyle.color }]}>
                <Text
                    style={[styles.meditationName(Grouding, item), { color: themeStyles.TextStyle.color }]}
                    numberOfLines={1}
                >
                    {item.title}
                </Text>
                <View style={styles.infoWrapper}>
                    <Text style={[styles.publisher(Grouding, item), { color: themeStyles.TextStyle.color }]}>
        ...
        <>
            <View style={styles.container} testID="popularContainer">
                <View style={[styles.header, { borderColor: themeStyles.TextStyle.color }]} testID="popularHeader">
                    <Text style={[styles.headerTitle, { color: themeStyles.TextStyle.color }]}>Grounding</Text>
...
const styles = StyleSheet.create({
    ...
    container: (Grouding, item) => ({
        width: 270,
        padding: SIZES.xLarge,
        marginHorizontal: SIZES.small,
        marginTop: SIZES.xLarge,
        marginBottom: SIZES.xLarge,
        backgroundColor: Grouding === item.id ? COLORS.primary : "#FFF",
    ...
    meditationName: (Grouding, item) => ({
        fontSize: SIZES.large,
        fontFamily: FONT.medium,
        color: Grouding === item.id ? COLORS.white : COLORS.primary,
    }),
    infoWrapper: {
        flexDirection: "row",
        marginTop: 5,
...
export default GroudingExercise;